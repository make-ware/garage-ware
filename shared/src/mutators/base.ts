...
export abstract class BaseMutator<T extends RecordModel, InputType> {
  ...
  protected async entityUpdate(id: string, data: Partial<T>): Promise<T> {
    return await this.getCollection().update(id, data, { requestKey: null });
  }
  ...
  protected async entityGetById(
    ...
    const options: RecordOptions = finalExpand
      ? { expand: finalExpand, requestKey: null }
      : { requestKey: null };
  ...
  protected async entityGetFirstByFilter(
    ...
    const options: RecordListOptions = { requestKey: null };
  ...
  protected async entityDelete(id: string): Promise<boolean> {
    await this.getCollection().delete(id, { requestKey: null });
    return true;
  }
   ...
   * PocketBase's ClientResponseError carries the HTTP status as a field;
   * its message ("The requested resource wasn't found.") does not contain the
   * literal substring "not found", so we check `status` first.
   */
