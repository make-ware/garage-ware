...
import { UserMutator } from '@garage-ware/shared';
...
```typescript
import { PostMutator, Post, PostInput } from '@garage-ware/shared';
```
...
3. Generates migration files in `pocketbase/pb_migrations/`
...
3. **Review migration** in `pocketbase/pb_migrations/` directory
4. **Apply migration**: Restart PocketBase or it will auto-apply on next start

Migrations are stored in `pocketbase/pb_migrations/` and handle:
