...
5. **Cluster event timeline** — a dated log of what changed in the cluster and why. Layout changes, disk resizes and connectivity are detected by diffing consecutive metrics scrapes; causes are typed in by an admin. See [Cluster events](#cluster-events).
...
`<NodeIdentity>` ([webapp/src/components/cluster/node-identity.tsx](webapp/src/components/cluster/node-identity.tsx)) is the admin-table treatment: name on top, short id beneath in muted mono, and for an unnamed node the short id **alone** — never the same string twice. It owns its own typography, so strip `font-mono text-xs` off the enclosing `<TableCell>`. Zone stays an attribute (its own column, the cluster-map grouping, the details-dialog badge) and is deliberately *not* part of any identifier — it came out of the node pickers for that reason. The one place a full, untruncated id belongs is the details dialog's `DialogDescription`; the node card's `aria-label` also uses it, since screen readers may announce a trailing ellipsis.
