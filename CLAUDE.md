...
- **The one exception is input, not output**: the claim form on `/dashboard/nodes`, where an operator pastes what `garage node id` printed. `<id>@<addr>` is accepted and split, since that is the form the command emits. `/admin/nodes` is *not* a second exception — it submits the key it already holds, and the route admits that from an admin; see [Node ownership](#node-ownership).
